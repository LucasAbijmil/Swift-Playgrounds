 ...
 * `actor` est un **nouveau concrete nominal type** en Swift comme les `struct`, `class` et `enum` :
    ...
    * Les `static property` et `static func` n'ont pas de conception du `self` et ne sont donc pas isolées
 ...
 * Tous les accès à un `actor` sont **synchronisés dans une File FIFO** (*First In First Out*), ce qui implique qu'**un `actor` travaille de manière sérialisée** (et potentiellement asynchrone)
 ...
 * Cela vient du fait *qu'on ne sait pas à quelle position on se trouve dans la File*
 */
/*:
...
/*:
 * Bien que les `actor` soient des references types, ils **ne supportent pas l'héritage** à l'inverse des `class` (sauf à `NSObject`). Ils le pourront **potentiellement** dans le futur.
